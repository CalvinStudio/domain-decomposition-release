...
_CG_QUALIFIER TyVal shuffle_reduce_pow2(const coalesced_group& group, TyVal val, TyOp op) {
    for (int offset = group.size() >> 1; offset > 0; offset >>= 1)
        val = op(val, group.shfl_down(val, offset));

    return val;
}

template <typename TyVal, typename TyOp>
_CG_QUALIFIER TyVal coalesced_reduce(const coalesced_group& group, TyVal val, TyOp op) {
    ...
    if (isPow2) {
        ...
        for (unsigned int i = 1, j = 1; i < groupSize; i <<= 1) {
            ...
            TyVal tmp = __shfl_sync(mask, val, srcLane);
            if (i <= rank) {
                val = op(val, tmp);
            }
        }
        // Redistribute the value after performing all the reductions
        return group.shfl(val, groupSize-1);
    }
    return val;
}

template <typename TyVal, typename TyOp, unsigned int TySize, typename ParentT>
_CG_QUALIFIER TyVal coalesced_reduce(const thread_block_tile<TySize, ParentT>& group, TyVal val, TyOp op) {
