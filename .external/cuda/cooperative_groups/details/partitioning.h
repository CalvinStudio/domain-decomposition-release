...
namespace details {
    ...
    public:
        ...
        _CG_STATIC_QUALIFIER coalesced_group construct_from_mask(unsigned int mask) {
            return coalesced_group(mask);
        }

        _CG_STATIC_QUALIFIER void modify_meta_group(coalesced_group &group, unsigned int mgRank, unsigned int mgSize) {
    ...
    _CG_STATIC_QUALIFIER coalesced_group _binary_partition(const TyGroup &tile, bool pred) {
        ...
        if (setMask == thisMask || setMask == 0) {
            coalesced_group subTile = _coalesced_group_data_access::construct_from_mask(thisMask);
            _coalesced_group_data_access::modify_meta_group(subTile, 0, 1);
            return subTile;
        }
        else {
            unsigned int subMask = thisMask & (setMask ^ predMask);
            coalesced_group subTile = _coalesced_group_data_access::construct_from_mask(subMask);
